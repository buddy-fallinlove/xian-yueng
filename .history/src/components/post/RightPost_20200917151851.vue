...
import { Res ,Centers, Dataitem} from "@/views/types/types";
import {
  defineComponent,
  reactive,
  toRefs,
...
import a from '../../../server/apidoc/vendor/jquery.min';
interface Data {
  postList: Dataitem[];
  current1: number;
  pageSize: number; // 页数
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    const change = (page: number, pageSize: number): void => {
        ...
        const postList = (city: any): void=>{
      api.getposte(city).then((res: Res)=>{
        console.log(res)
      })
    }
