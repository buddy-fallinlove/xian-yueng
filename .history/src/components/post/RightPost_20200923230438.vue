...
<div class="rightPost">
    <a-row class="sosuo_kuang" type="flex" justify="space-between" align="middle">
        <input type="text" v-model="value" ref="sosuoInput" @keyup.enter="sosuoClick" placeholder="请输入想去的地方，比如：'广州'" />
        <i @click="sosuoClick">
            <SearchOutlined />
        </i>
    </a-row>
    <div class="sosuo_tuijian">
        推荐：
        <span @click="clickeet" v-for="(item,index) in ['广州','上海','北京']" :key="index">{{item}}</span>
    </div>
    <!-- 标题 -->
    <a-row class="title" type="flex" justify="space-between" align="middle">
        ...
        <a-button type="primary">
            ...
            <span @click="goToWrite">写游记</span>
        </a-button>
    </a-row>
    <!-- 内容 -->
    <div class="post_item" @click="detail(item)" v-for="(item,index) in postList.slice((page-1)*pageSize,page*pageSize)" :key="index">
        <!-- 多张图片 -->
    ...
    <!-- 分页 -->
    <div class="fenye">
        <a-pagination :total="postList.length" show-size-changer show-quick-jumper :page-size-options="pageSizeOptions" v-model:current="current1" :page-size="pageSize" @change="change" @showSizeChange="showSizeChange" :show-total="total => `共 ${total} 条`">
...
export default defineComponent({
    ...
    setup(props, ctx: SetupContext) {
        ...
        const postList = (city: string): void => {
            api.getposte({
                city: city
            }).then((res: Centers) => {
                // console.log(res)
        ...
        const sosuoClick = (): void => {
            //  console.log(data.value)
            postList(data.value)
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const clickeet = (item: any): void => {
            console.log(item.toElement.innerHTML)
            data.value = item.toElement.innerHTML
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const detail = (item: any): void => {
            console.log(item)
            router.push({
                path: '/detail',
                query: {
