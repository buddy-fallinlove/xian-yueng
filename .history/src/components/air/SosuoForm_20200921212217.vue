...
<div class="search-form">
    ...
    <a-form v-show="currentTab===0" class="search-form-content" ref="form" label-width="80px">
        ...
        <!-- 搜索 -->
        <a-form-item>
            <a-button style="width:100%;" type="primary" @click="search">
                <i>
                    <SearchOutlined />
                </i>搜索
            </a-button>
...
export default defineComponent({
    ...
    setup(props, ctx: SetupContext) {
        ...
        const search = (): void => {
            if (data.value1 !== "" && data.value2 !== "" && data.value3 !== "") {
                api
                    .getCity({
                        name: data.value1
                    })
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    .then((res: any) => {
                        data.It.departCity = res.data[0].name;
                        data.It.departCode = res.data[0].sort;
                        console.log(data.It.departCity);
                        console.log(data.It.departCode);
                    })
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    .catch((err: any) => {
                        console.log(err);
                ...
                api
                    ...
                    .then((res: any) => {
                        console.log(res);
                        data.It.destCity = res.data[0].name;
                        data.It.destCode = res.data[0].sort;
                        console.log(data.It.destCity);
                        console.log(data.It.destCode);
                        router.push({
                            path: "/liebiao",
                            query: {
