    ...
    <a-row type="flex" class="filters-top" justify="space-between" align="middle">
        ...
        <a-col :span="4">
            <a-select v-model:value="flightTimes" style="width: 120px" placeholder="起飞时间">
                <a-select-option v-for="(item,index) in data.options.flightTimes" :key="index">
        ...
        <a-col :span="4">
            <a-select v-model:value="company" style="width: 120px" placeholder="航空公司">
                <a-select-option v-for="(item,index) in data.options.company" :key="index" :value="item">
                    {{item}}
                </a-select-option>
            </a-select>
        </a-col>
...
<div class="filter-cancel">
    筛选：
    <a-button type="primary" round plain size="mini">
        撤销
    </a-button>
</div>
</template>
