    ...
    <a-row type="flex" justify="center" align="middle" class="main">
        <div class="form-wrapper">
            ...
            <a-row type="flex" justify="center" class="tabs">
                <span :class="{active: current === index}" v-for="(item,index) in [`登录`,`注册`]" :key="index" @click="handleChangeTab(index)">{{item}}</span>
            </a-row>
            <!-- 登录功能组件 -->
            <div v-if="current == 0">
                <a-form :model="forms" ref="ruleForms" :rules="ruless" :label-col="labelCol" :wrapper-col="wrapperCol" class="form">
                    <!-- 用户名-->
            ...
            <div v-if="current == 1" @zhuce="current=0">
                <a-form :model="form" ref="ruleForm" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol" class="form">
                    <!-- 手机号码/用户名 输入框 -->
